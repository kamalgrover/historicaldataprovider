...
public class KiteSessionManager {
  ...
  KiteSessionManager(
      String apiKey, String apiSecret, int port, String callbackPath, String userId) {
    this.apiKey = apiKey;
  ...
  KiteConnect login() throws KiteSessionManagerException {
    ...
    try {
      ...
      throw new KiteSessionManagerException(
          "Possibly happens when the server thread is blocked", e);
    } catch (ExecutionException e) {
      throw new KiteSessionManagerException("don't know why this would occur", e);
    } catch (TimeoutException e) {
      throw new KiteSessionManagerException(
          "Server timed out while trying to fetch access token", e);
    }
